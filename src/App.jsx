...
function PostCard({ post }) {
  ...
  function handleLike(){setLiked(!liked);setCount(liked?count-1:count+1)}
  return (
    <article className="post">
      <div className="post-header">
        <div className="post-avatar" style={{background:post.avatarBg,color:post.avatarColor}}>{post.user[0]}</div>
        <div>
          <div className="post-name">{post.user}</div>
          <div className="post-handle">{post.handle} · {post.time}</div>
        </div>
      ...
      <div className="post-img" style={{background:post.imgBg}}>
        <span style={{fontSize:'88px'}}>{post.emoji}</span>
        <div className="post-img-overlay">
          <div>{post.badgeTop&&<span className={`badge ${post.badgeTop.cls}`}>{post.badgeTop.text}</span>}</div>
          <div style={{display:'flex',gap:'6px'}}>
            <span className="badge badge-gold">{post.badgeCar}</span>
            <span className="badge badge-dark">{post.badgeExtra}</span>
          </div>
        </div>
      </div>
      ...
      <div className="post-actions">
        <button className={liked?'act-btn liked':'act-btn'} onClick={handleLike}>{liked?'❤️':'🤍'} {count.toLocaleString()}</button>
        <button className="act-btn">💬 Comment</button>
        <button className="act-btn">↗ Share</button>
        <button className={saved?'act-btn save-btn saved':'act-btn save-btn'} onClick={()=>setSaved(!saved)}>{saved?'🔖 Saved':'🔖 Save'}</button>
      </div>
      <div className="post-body">
        <div className="post-likes">{count.toLocaleString()} revheads liked this</div>
        <p className="post-caption"><strong>{post.user}</strong> {post.caption} <span className="hashtag">{post.hashtags}</span></p>
      </div>
    </article>
  )
}
  ...
  return (
    <div className="app-layout">
      <div className="feed-col">
        <div className="feed-bar"><div><span className="feed-heading">RevUp</span><span className="feed-sub">Bangalore</span></div></div>
        ...
        <div className="chips">
          {CHIPS.map(c=><button key={c} className={chip===c?'chip active':'chip'} onClick={()=>setChip(c)}>{c}</button>)}
        </div>
